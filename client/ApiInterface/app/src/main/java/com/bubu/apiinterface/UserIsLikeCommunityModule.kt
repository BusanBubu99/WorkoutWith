...
class UserIsLikeCommunityModule(override val token: String, override val userData: JsonObject)
    : UserApiInterface<JsonObject,UserIsLikeCommunityResponseData> {
    ...
    override suspend fun getApiData(): UserIsLikeCommunityResponseData? {
        ...
        try {
            var resp = retrofitObject.get(20/*userData.postId*/).execute()
            if(resp.code() == OK) { //
